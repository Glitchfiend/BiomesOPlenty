...
public class BOPSurfaceSystem extends SurfaceSystem
{
    ...
    public void buildSurface(BiomeManager biomeManager, Registry<Biome> biomeRegistry, boolean useLegacyRandomSource, WorldGenerationContext p_189948_, final ChunkAccess chunkAccess, NoiseChunk noiseChunk, SurfaceRules.RuleSource ruleSource)
    {
        ...
        SurfaceRules.Context surfacerules$context = new SurfaceRules.Context(this, chunkAccess, noiseChunk, biomeManager::getBiome, biomeRegistry, p_189948_);
        ...
        for(int localX = 0; localX < 16; ++localX)
        {
            for(int localZ = 0; localZ < 16; ++localZ)
            {
                ...
                if (biomeKey == Biomes.ERODED_BADLANDS) {
                    this.erodedBadlandsExtension(blockcolumn, x, z, height, chunkAccess);
                }

                if (biomeKey == BOPBiomes.ORIGIN_VALLEY) {
                    this.originValleyExtension(chunkAccess, chunkRandom, biome, x, z, height);
                }
                ...
                if (biomeKey == Biomes.FROZEN_OCEAN || biomeKey == Biomes.DEEP_FROZEN_OCEAN) {
                    this.frozenOceanExtension(surfacerules$context.getMinSurfaceLevel(), biome, blockcolumn, blockpos$mutableblockpos1, x, z, height);
                }
            }
        }
    }
