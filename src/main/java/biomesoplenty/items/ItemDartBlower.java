...
public class ItemDartBlower extends Item
{
	...
	public ItemStack onItemRightClick(ItemStack itemStack, World world, EntityPlayer par3EntityPlayer)
	{
		...
		//TODO:								   hasItem()
		if (flag || par3EntityPlayer.inventory.func_146028_b(BOPItems.get("dart")))
		{
			...
			int slot = -1;
			if (par3EntityPlayer.inventory.hasItemStack(new ItemStack(BOPItems.get("dart"), 1, 1)))
			{
				entityDart.setDartType(DartType.POISON);

				for (int k = 0; k < par3EntityPlayer.inventory.mainInventory.length; ++k)
				{
					if (par3EntityPlayer.inventory.mainInventory[k] != null && par3EntityPlayer.inventory.mainInventory[k].getItem() == BOPItems.get("dart") && par3EntityPlayer.inventory.mainInventory[k].getItemDamage() == 1)
					{
						slot = k;
						break;
					}
			...
			else if (par3EntityPlayer.inventory.hasItemStack(new ItemStack(BOPItems.get("dart"), 1, 0)))
			{
				entityDart.setDartType(DartType.NORMAL);

				for (int k = 0; k < par3EntityPlayer.inventory.mainInventory.length; ++k)
				{
					if (par3EntityPlayer.inventory.mainInventory[k] != null && par3EntityPlayer.inventory.mainInventory[k].getItem() == BOPItems.get("dart") && par3EntityPlayer.inventory.mainInventory[k].getItemDamage() == 0)
					{
						slot = k;
						break;
					}
