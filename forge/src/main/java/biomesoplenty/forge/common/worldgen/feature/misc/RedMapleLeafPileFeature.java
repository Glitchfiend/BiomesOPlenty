...
{
    public RedMapleLeafPileFeature(Codec<NoneFeatureConfiguration> deserializer)
    {
        super(deserializer);
    }
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> featurePlaceContext)
    {
        ...
        for(int j = 0; j < 64; ++j)
        {
            ...
            if (TreeFeature.isAirOrLeaves(world, blockpos) && world.getBlockState(blockpos.below()).getBlock() == Blocks.GRASS_BLOCK)
            {
                for (int k = 0; k < 16; ++k)
                {
                    if (world.getBlockState(blockpos.above(k)).getBlock() == BOPBlocks.RED_MAPLE_LEAVES.get())
                    {
                        world.setBlock(blockpos, BOPBlocks.RED_MAPLE_LEAF_PILE.get().defaultBlockState(), 2);
                        ++i;
                    }
                }
            }
